...
%obj_proposals = {'Features/mscoco/train2014'};
%obj_proposals = {'Features/mscoco/val2014'};
obj_proposals = {'Features/mscoco/test2015'};
formats = {'.jpg'};



% Images subsampling for reducing the need of computational resources
subsampling = 1; % we can only process 1 out of 10 images

% Folder where to start extracting features
offset = 1;

object_proposals_name = 'EdgeBoxes'; % object proposals method used
nProposalsUse = 30; % number of object proposals used per image
...
CNN_params_ImageNet.batch_size = 140; % Depending on the deploy net structure!!
...
CNN_params_ImageNet.features_name = 'GoogleNet_ImageNet';
...
%addpath([CNN_params.caffe_path '/matlab/caffe']);
...
net = caffe.Net(CNN_params_ImageNet.model_def_file, CNN_params_ImageNet.model_file, 'test');
...
for f = offset:nFolders
    ...
    op = load([images_path '/' obj_proposals{f} '/object_proposals_' object_proposals_name '.mat']);
...
% 	% Initialize net
%         net = caffe.Net(CNN_params_Places.model_def_file, CNN_params_Places.model_file, 'test');
% 
%         for ib = 1:batch_size:tot_samples
%             this_batch = ib:min(ib+batch_size-1, tot_samples);
%             im_list = cell(1,batch_size);
